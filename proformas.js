...
const ProformasModule = (() => {
  ...
  const renderTable = (proformas) => {
    ...
    return `
      ...
      const validezDias = Number(proforma.validez_dias || proforma.validezDias) || 15;
      let diasRestantes = 0;
      let fechaVencimiento = null;
      if (fechaProforma) {
        fechaVencimiento = new Date(fechaProforma);
                ...
                <td data-label="Acciones">
                  <div class="flex gap-xs">
                    ...
                    ${proforma.estado === 'Activa' && canUpdate ? `
                      ...
                      <button class="btn btn--ghost btn--icon btn--sm text-success" onclick="ProformasModule.openAprobarModal('${proforma.proformaId || proforma.codigo_proforma}')" title="Aprobar & Facturar">
                        ${Icons.checkCircle}
                      </button>
                    ` : ''}
                    ${canDelete ? `
    ...
    return `
      <div class="modal-overlay open" onclick="ProformasModule.closeModal(event)">
        <div class="modal modal--lg" onclick="event.stopPropagation()">
          <div class="modal__header">
            <div>
              <h3 class="modal__title">Proforma PROF-${String(proformaNumero).padStart(4, '0')}</h3>
            </div>
            <button class="modal__close" onclick="ProformasModule.closeModal()">${Icons.x}</button>
          </div>
    ...
    const content = `
      <div class="header">
        ...
        <div class="proforma-info">
          <h2>PROFORMA</h2>
          <p><strong>Nº:</strong> PROF-${String(proformaNumero).padStart(4, '0')}</p>
  ...
  const evaluarCredito = (total) => {
    const montoInput = parseFloat(document.getElementById('montoPagadoInput')?.value) || 0;
    const saldo = Math.max(0, total - montoInput);
    const container = document.getElementById('creditoContainer');
    const span = document.getElementById('saldoPendienteSpan');
    if (container && span) {
      if (saldo > 0.01) {
        container.style.display = 'block';
        span.textContent = saldo.toFixed(2);
      } else {
  ...
  const openAprobarModal = (proformaId) => {
    const proforma = DataService.getProformaById(proformaId);
    if (!proforma) return;
  ...
  const aprobarProforma = async (proformaId) => {
    const formaPago = document.getElementById('formaPagoSelect')?.value || 'Efectivo';
    const montoPagado = parseFloat(document.getElementById('montoPagadoInput')?.value) || 0;
    const plazoCredito = parseInt(document.getElementById('plazoCreditoInput')?.value) || 0;

    try {
      await DataService.aprobarProforma(proformaId, { formaPago, montoPagado, plazoCredito });
      App.refreshCurrentModule();
      closeModal();
      if (typeof App.showNotification === 'function') {
        App.showNotification('Proforma aprobada y Factura generada', 'success');
      }

      if (confirm('Factura generada correctamente. ¿Deseas ir al módulo de facturas para verla/imprimirla?')) {
        App.navigate('facturas');
      }
    } catch (error) {
